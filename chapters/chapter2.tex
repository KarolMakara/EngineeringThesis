...
StatefulSet unlike \textit{\nameref{deployment}} is stateful. It saves an identity of each pod and if e.g., some persistent storage is assigned to specific e.g., database pod, when it dies, Kubernetes will recreate pod on the same node as it was previously \cite{KubernetesStatefulSet}.
...
\label{cronjob}
Behaviors like \textit{\nameref{job}} but can regularly run every given time for tasks like database backups or log rotation \cite{KubernetesCronJob}. 
