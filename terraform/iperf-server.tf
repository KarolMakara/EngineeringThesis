      ...
      spec {
        container {
          ...
          command = [
            "/bin/sh",
            "-c",
            "/usr/local/bin/statexec -s --log-file ${local.iperf_server_json_path} --file ${local.statexec_iperf_server_metrics_path} -l cni=${var.CNI_NAME} -mst ${var.metrics_reference_time} -- /usr/local/bin/iperf3 -s --json"
          ]
